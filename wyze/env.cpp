#include "env.h"
#include "log.h"
#include "config.h"
#include <stdio.h>
#include <unistd.h>
#include <string.h>
#include <iostream>
#include <iomanip>

namespace wyze {

static Logger::ptr g_logger = WYZE_LOG_NAME("system");

bool Env::init(int argc, char** argv)
{
    char link_path[1024] = {0};
    char link_content[1024] = {0};
    snprintf(link_path,sizeof(link_path), "/proc/%d/exe", getpid());
    int rt = readlink(link_path, link_content, sizeof(link_content));
    (void)rt;

    m_exe = link_content;
    auto pos = m_exe.find_last_of("/");
    m_cwd = m_exe.substr(0, pos + 1);
    m_program = argv[0];

    const char* now_key = nullptr;
    for(int i = 1; i < argc; ++i) {
        if(argv[i][0] == '-') {     //这是一个key
            if(strlen(argv[i]) > 1) {
                if(now_key)     //上一个key 没有value
                    add(now_key, "");
                now_key = argv[i] + 1;
            }
            else {
                WYZE_LOG_ERROR(g_logger) << "invalid arg idx=" << i
                    << " val=" << argv[i];
                return false;
            }

        }
        else {  //这是一个value
            if(now_key) {
                add(now_key, argv[i]);
                now_key = nullptr;
            }
            else {
                WYZE_LOG_ERROR(g_logger) << "invalid arg idx=" << i
                    << " val=" << argv[i];
                    return false;
            }
        }
    }
    if(now_key)
        add(now_key, "");
    
    return true;
}

void Env::add(const std::string& key, const std::string& val)
{
    RWMutexType::WriteLock wlock(m_mutex);
    m_args[key] = val;
}

bool Env::has(const std::string& key)
{
    RWMutexType::ReadLock rlock(m_mutex);
    auto it = m_args.find(key);
    return it != m_args.end();
}

void Env::del(const std::string& key)
{
    RWMutexType::WriteLock wlock(m_mutex);
    m_args.erase(key);
}

std::string Env::get(const std::string& key, const std::string& default_value)
{
    RWMutexType::ReadLock rlock(m_mutex);
    auto it = m_args.find(key);
    return it != m_args.end() ? it->second : default_value;
}

void Env::addHelp(const std::string& key, const std::string& desc)
{
    removeHelp(key);
    RWMutexType::WriteLock wlock(m_mutex);
    m_helps.push_back(std::make_pair(key, desc));
}

void Env::removeHelp(const std::string& key)
{
    RWMutexType::WriteLock wlock(m_mutex);
    for(auto it = m_helps.begin();
        it != m_helps.end(); ) {
        if(it->first == key)
            it = m_helps.erase(it);
        else 
            ++it;
    }
}

void Env::printHelp()
{
    RWMutexType::ReadLock rlock(m_mutex);
    std::cout << "Usage: " << m_program << " [options]" << std::endl;
    for(auto& i : m_helps) {
        std::cout << std::setw(5) <<"-" << i.first << " : " << i.second << std::endl;
    }
}

bool Env::setEnv(const std::string& key, const std::string& val)
{
    return !setenv(key.c_str(), val.c_str(), 1);
}

std::string Env::getEnv(const std::string& key, const std::string& default_value)
{
    const char* v = getenv(key.c_str());
    if(v == nullptr)
        return default_value;
    return v;
}

std::string Env::getAbsolutePath(const std::string& path) const
{
    if(path.empty())
        return "/";

    if(path[0] == '/')
        return path;

    return m_cwd + path;
}

std::string Env::getAbsoluteWorkPath(const std::string& path) const
{
    if(path.empty())
        return "/";

    if(path[0] == '/')
        return path;

    static ConfigVar<std::string>::ptr s_server_work_path =
        Config::Lookup<std::string>("server.work_path");

    return s_server_work_path == nullptr ? nullptr : s_server_work_path->getValue() + "/" + path;
}

std::string Env::getConfigPath()
{
    // static std::string defalt_value("conf");
    return getAbsolutePath(get("c", "conf"));
}

}